...
export class RenderQueue {
	...
	//Added by Sebastien
	/*openTextureInNewTab(name){//DEPRECATED
		const reference = this._textureMap[name];
		this._renderer.openTextureInNewTab(reference);
	}*/
	takeScreenshot(name, segmented = false, sizeMultiplier = 1){
		const reference = this._textureMap[name];
		this._renderer.takeScreenshot(segmented, sizeMultiplier, this, reference);
	}

...
export class LASLoader {
    constructor (manager = new RC.LoadingManager(), responseType = "", stream = false, chunkSize = 1024*1024) {
    ...
    load (url, set, onLoad, onProgress, onError, onAbort, onHeaderLoad, onLoadChunk) {
        if(!this._stream){
            //TODO
            //this._loadNormal(url, onLoad, onProgress, onError, onAbort);
            console.error("LAS non-stream mode is not supported at this moment.");
        }else if(this._stream){
            this._loadStream(url, set, onLoad, onProgress, onError, onAbort, onHeaderLoad, onLoadChunk);
        }
    }

    _loadNormal (url, onLoad, onProgress, onError) {
        const loader = new RC.XHRLoader(this._manager, this._responseType);

...
float rand_constant(vec4 p) {
    return fract(sin(mod(dot(p.xyzw, vec4(12.9898, 78.233, 144.712, 5.1337)), 3.14)) * 43758.5453);
}
...
float perlinNoise4D(vec4 p) {
	...
	return w_xyzw * M_SQRT3; // That's how Ken Perlin does it
}

vec3 getPotentialP(vec4 p) {
	return vec3(
		perlinNoise4D(p),
		perlinNoise4D(vec4(p.z - 9.61, p.x + 3.14, p.y + 9.77, p.w)),
		perlinNoise4D(vec4(p.y - 2.32, p.z + 12.7, p.x + 11.97, p.w))
	);
}

vec3 getVelocityP(vec3 pos, float time) {
	vec4 p = vec4(pos / uFlowScale, time * uFlowEvolution);
	...
	return vec3(dp3_dy - dp2_dz, dp1_dz - dp3_dx, dp2_dx - dp1_dy) * uFlowSpeed;
}
...
vec3 getPotentialI(vec4 p) {
	return vec3(
		improvedNoise4D(p),
		improvedNoise4D(vec4(p.z - 9.61, p.x + 3.14, p.y + 9.77, p.w)),
		improvedNoise4D(vec4(p.y - 2.32, p.z + 12.7, p.x + 11.97, p.w))
	);
}
...
vec3 getPotentialS(vec4 p) {
	return vec3(
		snoise(p),
		snoise(vec4(p.z - 9.61, p.x + 3.14, p.y + 9.77, p.w)),
		snoise(vec4(p.y - 2.32, p.z + 12.7, p.x + 11.97, p.w))
	);
}
...
void writeData(Particle p, int texelOffset) {
	if (texelOffset == 0) { // Write position
		oWrite = vec4(p.pos, 1.0);
	} else if (texelOffset == 1) { // Write velocity
		oWrite = vec4(p.life, p.age, p.seed, 1.0);
	}
}
...
void main() {
	...
	if (p.life > 0.0) { // Update particle
		// vec3 vel = getVelocityP(p.pos, uTime);
		// vec3 vel = getVelocityI(p.pos, uTime);
		vec3 vel = getVelocityS(p.pos, uTime);
		p.pos += vel * uDT;
		p.age += uDT;
	} else { // Respawn particle
		p.pos = sampleUnitSphere() * uSpawnRadius + uCameraPos;
		p.life = rand() * (uLifespan.y - uLifespan.x) + uLifespan.x;
		p.age = 0.0;
	}
